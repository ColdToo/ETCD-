...
type raft struct {
	...
	//这时它自己不能发现自己已不是 Leader。Check Quorum 机制可以帮助 Leader 主动发现这种情况：每当 Election Timeout 时检查一次，
...
func (r *raft) Step(m pb.Message) error {
	...
	case m.Term == 0:
		...
		//如果接收到一条任期大于自身的消息，需要判断是否需要退位回 follower 状态.
	case m.Term > r.Term:
		if m.Type == pb.MsgVote || m.Type == pb.MsgPreVote {
			force := bytes.Equal(m.Context, []byte(campaignTransfer)) // 判断是否是leader转移的消息
		...
		//倘若收到了一条任期小于自身的消息，需要对心跳或者日志同步请求回复自身的任期，帮助老 leader 尽快退位.
